...
class EqMixin:
    ...
    def compare_value(self):
        ...
        return inspect(type(self)).identify
...
class DeclarativeMeta(BaseDeclarativeMeta):
    def __init__(cls, name, bases, attrs):
        """Handle FGlask-SQLAlchemy's bind_key without setting tablename."""
...
class SGSQLAlchemy(BaseSQLAlchemy):
    def __init__(self, app=None, model=Model, meta=DeclarativeMeta, **kwargs):
        self.BaseModel = model
        self.DeclarativeMeta = meta
        super(SGSQLAlchemy, self).__init__(app, **kwargs)

    def init_app(self, app):
        super(SGSQLAlchemy, self).init_app(app)
        app.shell_context_processor(lambda: {'db': self})

    def make_declarative_base(self, metadata=None):
        metadata = MetaData(naming_convention={
            'pk': 'pk_%(table_name)s',
            'fk': 'fd_%(table_name)s_%(column_0_name)s_%(referred_table_name)s',
